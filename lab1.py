...
# рисуем вектор ускорения в нулевой момент времени (в anima будем рисовать с 1 до последнего момента времени)
a_line, = ax1.plot([X[0], X[0] + AX[0]], [Y[0], Y[0] + AY[0]], 'g')
...
# Координаты концов вектора ускорения
ArrowAX = np.array([-0.2 * Radius, 0, -0.2 * Radius])
ArrowAY = np.array([0.1 * Radius, 0, -0.1 * Radius])


RArrowX, RArrowY = Rot2D(ArrowX, ArrowY, math.atan2(VY[0], VX[0]))
RArrowRX, RArrowRY = Rot2D(ArrowRX, ArrowRY, math.atan2(X[0], Y[0]))
# Поворот вектора ускорения
...
# Отображение вектора ускорения на графике
...
def anima(j):
    ...
    a_line.set_data([X[j], X[j] + AX[j]], [Y[j], Y[j] + AY[j]])#
    ...
    return P, v_line, VArrow, v_line3, RArrow, a_line#
